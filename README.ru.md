...
```bash
git clone <URL-вашего-репозитория> PygamE
cd PygamE
...
**Управление:** `A` / `D` или `←` / `→` — движение, `Space` — прыжок (второй раз — в воздухе), `Esc` закрывает окно, если вы обработаете его сами (см. [Input](#input)). **Отладка:** `F1` — статистика, `F2` — коллайдеры, `F3` — сетка мира, `F4` — консоль логов.
...
3. Физика идёт фиксированным шагом, `PlayerController` читает ввод каждый кадр ([5.1](#51-rigidbody2d), [Input](#input)).
